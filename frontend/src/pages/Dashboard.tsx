...
import {
  Send, Loader2, CheckCircle2, XCircle, BarChart3, Brain,
  FileText, RefreshCw, TrendingUp, Users, Mail, Sparkles,
} from "lucide-react";
      ...
      <div className="container max-w-5xl">
        ...
        <div className="flex items-center justify-center gap-3 mb-12">
          {[
            ...
            { label: "Analytics", icon: BarChart3, active: state === "analytics" || state === "approved-polling" },
          ].map((step, i) => (
            <div key={step.label} className="flex items-center gap-3">
              {i > 0 && <div className={`w-12 h-px ${step.active || state === "analytics" ? "bg-primary" : "bg-border"}`} />}
              <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                step.active ? "bg-primary/10 text-primary border border-primary/30" : "text-muted-foreground border border-border/50"
              }`}>
          ...
          {state === "brief" && (
            <motion.div key="brief" {...pageVariants} className="max-w-2xl mx-auto">
              <div className="text-center mb-8">
                ...
                <p className="text-muted-foreground">Describe your BFSI marketing campaign and let the AI take over.</p>
              </div>
          ...
          {state === "polling" && (
            <motion.div key="polling" {...pageVariants} className="max-w-2xl mx-auto">
              <div className="text-center mb-8">
                ...
                <p className="text-muted-foreground">Generating segments, crafting emails, and optimizing variants...</p>
              </div>
          ...
          {state === "approval" && campaignData && (
            <motion.div key="approval" {...pageVariants}>
              <div className="text-center mb-8">
                ...
                <p className="text-muted-foreground">The AI has generated the following campaign variants for your review.</p>
              </div>
              ...
              {campaignData.segments && campaignData.segments.length > 0 && (
                <div className="flex flex-wrap items-center justify-center gap-2 mb-8">
                  ...
                  {campaignData.segments.map((seg) => (
                    <span key={seg} className="px-3 py-1 rounded-full text-xs font-medium bg-primary/10 text-primary border border-primary/20">
                      {seg}
                    </span>
                  ))}
                </div>
                  ...
                  <div key={i} className="glass rounded-2xl overflow-hidden">
                    <div className="p-5 border-b border-border/50">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Variant {String.fromCharCode(65 + i)}</span>
                        {(variant.tone || (variant as any).segment_id) && <span className="px-2 py-0.5 rounded text-xs font-medium bg-accent/10 text-accent">{variant.tone || (variant as any).segment_id}</span>}
                      </div>
          ...
          {state === "approved-polling" && (
            <motion.div key="approved-polling" {...pageVariants} className="max-w-2xl mx-auto text-center">
              <div className="mb-8">
                ...
                <p className="text-muted-foreground">Running A/B tests and gathering performance data...</p>
              </div>
          ...
          {state === "analytics" && campaignData && (
            <motion.div key="analytics" {...pageVariants}>
                      ...
                      <div className="space-y-4">
                        <div>
                          <div className="flex items-center justify-between text-sm mb-1">
                            ...
                            <span className="font-semibold text-foreground">{((report.open_rate || 0) * 100).toFixed(1)}%</span>
                          </div>
                          <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
                            <div className="h-full bg-primary rounded-full" style={{ width: `${(report.open_rate || 0) * 100}%` }} />
                          </div>
                        </div>
                        <div>
                          <div className="flex items-center justify-between text-sm mb-1">
                            <span className="text-muted-foreground">Click Rate</span>
                            <span className="font-semibold text-foreground">{((report.click_rate || 0) * 100).toFixed(1)}%</span>
                          </div>
                          <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
                            <div className="h-full bg-accent rounded-full" style={{ width: `${(report.click_rate || 0) * 100}%` }} />
                          </div>
                        </div>
                        <div className="pt-2 border-t border-border/50 flex items-center justify-between">
                          <span className="text-sm text-muted-foreground">Composite Score</span>
                          <span className="text-xl font-bold text-gradient">{((report.composite_score || 0) * 100).toFixed(1)}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="glass rounded-2xl p-8 gradient-border">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Strategic optimization insights</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    {campaignData.optimization_history.map((entry, i) => (
                      <div key={i} className="flex gap-4 p-4 rounded-xl bg-secondary/30 border border-border/30">
                        ...
                        <div className="space-y-1">
                          {entry.action && <p className="text-sm font-medium text-foreground">{entry.action}</p>}
                          {entry.reasoning && <p className="text-sm text-muted-foreground">{entry.reasoning}</p>}
                          {entry.timestamp && <p className="text-xs text-muted-foreground/60">{entry.timestamp}</p>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="text-center mt-10">
                <button
                  onClick={() => { setState("brief"); setBrief(""); setCampaignData(null); setThreadId(""); setIteration(1); }}
